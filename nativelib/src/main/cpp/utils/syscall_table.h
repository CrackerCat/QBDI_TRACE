//
// Created by 14473 on 2024/4/23.
//

#ifndef FREELY_SYSCALL_TABLE_H
#define FREELY_SYSCALL_TABLE_H
...
static const syscall_item_t syscall_tbl[] = {
        {"__NR_accept", 202},
        {"__NR_accept4", 242},
        {"__NR_acct", 89},
        {"__NR_add_key", 217},
...
// 计算 syscall 表大小
#define SYSCALL_TBL_SIZE (sizeof(syscall_tbl) / sizeof(syscall_tbl[0]))

// 查找 syscall 名称
const char *get_syscall_name(int syscall_id) {
    for (size_t i = 0; i < SYSCALL_TBL_SIZE; i++) {
        if (syscall_tbl[i].id == syscall_id) {
            return syscall_tbl[i].name;
        }
    }
    return "unknown_syscall"; // 未找到对应的 syscall
}

// 获取 syscall ID 通过 name
int get_syscall_id_by_name(const char *name) {
    for (size_t i = 0; i < sizeof(syscall_tbl) / sizeof(syscall_tbl[0]); i++) {
        if (strcmp(syscall_tbl[i].name, name) == 0) {
            return syscall_tbl[i].id;  // 返回 syscall ID
        }
    }
    return -1;  // 未找到返回 -1
}
